...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if Auth.auth().currentUser != nil {
            //listener.を登録して投稿データの更新を監視する
            let postsRef = Firestore.firestore().collection(Const.PostPath).order(by: "date", descending: true)
            listener = postsRef.addSnapshotListener() { (quenrySnapshot, error) in
                if let error = error {
                    print("DEBUG_PRINT: snapshotの取得が失敗しました。　\(error)")
                    return
                }
                // 取得したdocumentをもとにPostDataを作成し、postArrayの配列にする。
                self.postArray = quenrySnapshot!.documents.map { document in
    ...
    //ホーム画面が閉じられるときにremove() メソッドを実行することで監視を停止させる。
    ...
    @objc func handleButton(_ sender: UIButton, forEvent event: UIEvent) {
        ...
        if let myid = Auth.auth().currentUser?.uid {
            ...
            if postData.isLiked {
                //すでにいいねをしている場合：いいね解除のためmyidを取り除く更新データを作成する。
                updateValue = FieldValue.arrayRemove([myid])
            } else{
                //新規いいねの場合：myidを追加する更新データを作成
